...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "my-rhpam-project-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name my-rhpam-project-service apps/my-rhpam-project-service:1.0.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app my-rhpam-project-service:1.0.0-SNAPSHOT
		call oc expose svc/my-rhpam-project-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd my-rhpam-project-service
		cd target

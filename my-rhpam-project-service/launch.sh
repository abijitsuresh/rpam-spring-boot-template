...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "my-rhpam-project-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name my-rhpam-project-service apps/my-rhpam-project-service:1.0.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app my-rhpam-project-service:1.0.0-SNAPSHOT
    oc expose svc/my-rhpam-project-service
else

	echo "Launching the application locally..."
	pattern="my-rhpam-project-service"
	files=( $pattern )
	cd ${files[0]}
